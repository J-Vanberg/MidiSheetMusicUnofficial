...
namespace MidiSheetMusic
{
    ...
    public class SheetMusic : Control
    {
        ...
        public bool EnableSmoothScroll = false; // Enable smooth scrolling

        private static List<Staff> staffs;      // The array of staffs to display (from top to bottom)
        ...
        private SolidBrush shade2Brush;         // The brush for shading the piano
        private Pen pen;                        // The black pen for drawing
        private int curShadePulseTime = -10;    // The current time pulse to shade to
        private int prevShadePulseTime = -10;   // The previous time pulse to shade from
        private bool smoothScroll = true;       // Scroll smoothly (gradually)
        private Timer UI_Tim_AutoScroll;
        private System.ComponentModel.IContainer components;
        private bool disablePaint = false;      // Prevents the OnPaint event from firing the draw routine
        #endregion
         ...
         */
        private List<ChordSymbol> CreateChords(List<MidiNote> midinotes, KeySignature key, TimeSignature time, ClefMeasures clefs)
        {
         ...
         */
        private List<MusicSymbol> CreateSymbols(List<ChordSymbol> chords, ClefMeasures clefs, TimeSignature time, int lastStart)
        {
         ...
         * starttime.
         */
        private void AlignSymbols(List<MusicSymbol>[] allsymbols, SymbolWidths widths, MidiOptions options)
        {
        ...
        /// <summary>
        /// Calls the Draw() function provided painting is not disabled.
        /// </summary>
        /// <param name="e">Standard paint arguments, provides access to graphics and canvas rectangle.</param>
        ...
        protected void Draw()
        {
            // Don't draw if we have been orphaned,
            // this would otherwise cause errors on determining our canvas and autoscroll
            if (Parent == null)
            {
                return;
            }

            // Determine the size and location of our canvas
            Panel parentPanel = (Panel)Parent;
            Rectangle canvas = new Rectangle(
                    Parent.ClientRectangle.X + -parentPanel.AutoScrollPosition.X - 150,
                    Parent.ClientRectangle.Y + -parentPanel.AutoScrollPosition.Y,
                    Parent.ClientRectangle.Width + 150,
            ...
            {
                using (BufferedGraphics bg = bgc.Allocate(CreateGraphics(), canvas))
                {
                    // Clear
                    bg.Graphics.Clear(Color.White);

                    Rectangle clip =
                      new Rectangle((int)(canvas.X / zoom),
                                    (int)(canvas.Y / zoom),
                    ...
                    foreach (Staff staff in staffs)
                    {
                        ...
                        else
                        {
                            ...
                            staff.ShadeNotes(g, shadeBrush, pen, curShadePulseTime, prevShadePulseTime, ref x_shade);
                            staff.Draw(g, clip, pen);
                            g.TranslateTransform(0, -ypos);
                        }

                        if (curShadePulseTime >= staff.EndTime)
                        {
        ...
        /// <summary>
        /// Forcibly redraws the sheet music without flicker caused by Invalidate().
        /// </summary>
        ...
        /// </summary>
        /// <param name="g">Graphics to print onto.</param>
        /// <param name="pagenumber">Page to print.</param>
        ...
        //  to the shaded notes.
        // JV: Instead set pulse variables on the SheetMusic object, handle
        //  shading as part of rendering (OnPaint)
        public void ShadeNotes(int currentPulseTime, int prevPulseTime, bool scrollGradually)
        {
            curShadePulseTime = currentPulseTime;
            prevShadePulseTime = prevPulseTime;
            smoothScroll = scrollGradually && EnableSmoothScroll;

            // Force a repaint
            Draw();

            if (curShadePulseTime >= 0 && scrollGradually)
            {
                ScrollToShadedNotes(smoothScroll);
            }
        }
        ...
        /** Scroll the sheet music so that the shaded notes are visible.
          ...
          * to the shaded notes.
          */
        void ScrollToShadedNotes(bool scrollGradually)
        {
            ...
            // Determine scroll destination
            int scrollDestination = 0;
            foreach (Staff staff in staffs)
            {
                if (scrollVert && curShadePulseTime >= staff.EndTime)
                {
                    scrollDestination += staff.Height;
                }

                if (!scrollVert)
                {
                    scrollDestination = staff.GetHorizontalScrollDestination(curShadePulseTime, prevShadePulseTime);
                }
            }

            if (scrollVert)
            {
                scrollDestination -= NoteHeight;
            }
            scrollDestination = (int)(scrollDestination * zoom);

            if (scrollVert)
            {
                int scrollDist = scrollDestination - scrollPos.Y;
            ...
            else
            {
                ...
                int scrollDist = scrollDestination - x_view;

                if (scrollGradually)
                {
                    if (scrollDestination > xmax)
                        scrollDist = (scrollDestination - x_view) / 6;
                    else if (scrollDestination > x_view)
                        scrollDist = (scrollDestination - x_view) / 12;
                }
        ...
        private void InitializeComponent()
        {
            ...
            this.UI_Tim_AutoScroll = new System.Windows.Forms.Timer(this.components);
            this.SuspendLayout();
            // 
            // UI_Tim_AutoScroll
            // 
            this.UI_Tim_AutoScroll.Enabled = true;
            this.UI_Tim_AutoScroll.Interval = 14;
            this.UI_Tim_AutoScroll.Tick += new System.EventHandler(this.UI_Tim_AutoScroll_Tick);
            this.ResumeLayout(false);

        }

        private void UI_Tim_AutoScroll_Tick(object sender, EventArgs e)
        {
            if (curShadePulseTime >= 0 && smoothScroll)
            {
                ScrollToShadedNotes(smoothScroll);
            }
        }
