...
namespace MidiSheetMusic
{
    ...
    public class Volume
    {
        ...
        /// </summary>
        /// <returns>The current system volume, range 0-100.</returns>
        public static int GetVolume()
        {
            if (Type.GetType("Mono.Runtime") != null)
            {
                // JV: I have no plans to personally use this on Linux, simply use
                // the existing default of 100
                return 100;
            }
            else if (Environment.OSVersion.Version.Major == 5)
            {
                // JV: WinXP is not supported by Microsoft, I won't either
                return 100;
            }
            else
            {
                // JV: This should prevent anyone from losing their hearing.
                // The application used to set the system volume to 100 on start,
                // which would be really bad for headphone and laptop users
                return GetVolumeWindowsVista();
            }
        }
