...
namespace MidiSheetMusic
{
    ...
    public class MidiPlayer : Panel
    {
        ...
        private MidiPlayState playstate; // The playing state of the Midi Player
        public MidiPlayState PlayState
        {
        ...
        public void SetMidiFile(MidiFile file, MidiOptions opt, SheetMusic s)
        {
             ...
             * highlighted notes.
             */
            if ((file == midifile && midifile != null && playstate == MidiPlayState.Paused))
            {
                options = opt;
                sheet = s;
        ...
        {
            if (playstate == MidiPlayState.Paused)
            {
                sheet.ShadeNotes((int)currentPulseTime, (int)-10, false);
        ...
        {
            if (midifile == null || sheet == null || playstate != MidiPlayState.Paused)
            {
                return;
            }
        ...
        {
            if (playstate == MidiPlayState.Playing)
            {
                Volume.SetVolume(volumeBar.Value);
            }
        }
